...
export default function Home() {
  ...
  const [selectPackageNamespace, setSelectPackageNamespace] =
    useState(defaultNull);
  ...
  useEffect(() => {
    ...
    processedPaths = processedPaths.map((l) =>
      l.map((x: number) => x.toString())
    );
  ...
  useEffect(() => {
    ...
    async function fetchNodeAndProcess(
      nodeId: string,
      nodeSet: Set<string>
    ): Promise<[Map<string, Node>, Map<string, Edge>]> {
      ...
      if (!IsSwTreeNode(n)) {
        ...
        filteredEdges = parsedGd.edges.filter(
    ...
    const promises = inputPath.map((path) =>
      path.map((nid) => fetchNodeAndProcess(nid, new Set(path)))
    );
          ...
          {graphData.map((d) => {
            ...
            return (
              <Graph
                key={d.key}
                layout="cola"
                writeDetails={writeDetailsHandler}
                graphData={d.gd}
              />
            );
          })}
